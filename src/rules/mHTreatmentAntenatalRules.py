...
def create_mental_health_antenatal(request: Request, mentalhealthAntenatal: MentalHealthAntenatalModel = Body(...)):
...
def find_one_mental_health_antenatal(request: Request, id: str):
    if mental := getcollection_mental_health_antenatal(request).find_one({"id": ObjectId(id)}):
        return mental
