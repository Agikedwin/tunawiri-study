...
@router.post("/", response_description="Create a new socialsupport", status_code=status.HTTP_201_CREATED,
             response_model=SocialSupportModel)
def create_social_support(request: Request, socialsupport: SocialSupportModel = Body(...)):
    socialsupport.created_at = datetime.now()
...
@router.get("/{id}", response_description="Get a single Social Support Rules by id",
            response_model=List[SocialSupportModel])
